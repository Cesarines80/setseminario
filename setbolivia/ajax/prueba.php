...
switch ($_GET["op"]) {
    ...
    //  var_dump($semestre);
        if (empty($id_reg)) {
            ...
            //id_reg,id_alm,id_mat,fecha,id_doc,id_grupo,id_ciudad,activo == registro
        //id_his,id_alm,id_mat,nota,fecha,id_reg,docente,activo == notas_historial
    ...
    case 'listar':
            ...
            while ($reg = $rspta->fetch_object()) {
                $data[]=array(
                    "0"=>($reg->activo) ?'<buton class="btn btn-warning" id="mostrar" onclick="mostrar('.$reg->id_reg.')"><i class="fa fa-pencil"></i></buton>'.
